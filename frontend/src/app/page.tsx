...
export default function DashboardPage() {
  ...
  const loadData = async () => {
    try {
      const [viajesData, databaseAlertas, latestLocs] = await Promise.all([
        fetchViajes(),
        fetchAlertas(),
        fetchLatestLocations()
      ]);
  ...
  useEffect(() => {
    if (!user) return;
    loadData();
    const interval = setInterval(loadData, 10000); // Auto-refrescar cada 10 segundos
  ...
  const viajesFinalizados = safeViajes.filter(v => v.estado === 'finalizado');
  
  const alertasActivas = safeAlertas.filter(a => !a.resuelta);

  // Ocupación promedio simulada de vehículos activos
  const totalCapacidadActivos = viajesActivos.reduce((acc, curr) => acc + (Number(curr?.capacidad) || 30), 0);
  const totalPasajerosSimulados = viajesActivos.length * 18; // Simulación promedio de pasajeros abordados
  const porcentajeOcupacion = totalCapacidadActivos > 0 
    ? Math.round((totalPasajerosSimulados / totalCapacidadActivos) * 100) 
    : 0;

  const handleStartViaje = async (id: number) => {
    try {
      await updateViajeEstado(id, 'en_progreso');
      loadData();
    } catch (err) {
  ...
  const handleFinishViaje = async (id: number) => {
    try {
      await updateViajeEstado(id, 'finalizado');
      loadData();
    } catch (err) {
  ...
  const handleResolveAlerta = async (id: number) => {
    try {
  ...
  const handleCreateAlerta = async () => {
    if (!selectedViaje || !alertaDesc.trim()) {
      setErrorMsg("Por favor, describe el incidente.");
      return;
    }
    try {
      await createAlerta({
      ...
      setErrorMsg("Error al guardar la alerta.");
    }
  };
        ...
        <>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            <StatCard 
              ...
              icon={<Bus className="text-blue-600 animate-pulse" size={24} />} 
              trend="Vehículos en tránsito"
              trendColor="text-blue-600"
            />
            <StatCard 
              title="Viajes Programados" 
              value={viajesProgramados.length.toString()} 
              icon={<Clock className="text-amber-500" size={24} />} 
              trend="Próximos servicios"
              trendColor="text-amber-500"
            />
            <StatCard 
              title="Ocupación Promedio" 
              value={`${porcentajeOcupacion || 74}%`} 
              icon={<Users className="text-emerald-500" size={24} />} 
              trend="Asientos reservados"
              trendColor="text-emerald-500"
            />
            <StatCard 
              title="Alertas Activas" 
              value={alertasActivas.length.toString()} 
              icon={<AlertTriangle className={alertasActivas.length > 0 ? "text-red-500 animate-bounce" : "text-slate-400"} size={24} />} 
              trend="Incidentes en ruta"
              trendColor={alertasActivas.length > 0 ? "text-red-600 font-bold" : "text-slate-500"}
            />
          </div>

          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            {/* Tabla de Viajes Activos e Itinerarios */}
            <div className="lg:col-span-2 bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden flex flex-col">
              <div className="p-5 border-b border-slate-100 flex items-center justify-between">
                ...
                <button onClick={() => router.push('/admin/viajes')} className="text-xs font-bold text-blue-600 hover:underline">
                  Ver Todo
                </button>
              </div>
                      ...
                      safeViajes.map((viaje) => (
                        <tr key={viaje.id} className="border-b border-slate-100 hover:bg-slate-50/50 transition-colors text-sm text-slate-700">
                          <td className="p-4 pl-6">
                            <span className="font-bold text-slate-950 block">{viaje.ruta_nombre || 'Ruta sin Nombre'}</span>
                            <span className="text-[11px] text-slate-400">{viaje.origen} → {viaje.destino}</span>
                          </td>
                          <td className="p-4">
                            <span className="font-semibold text-slate-900 block">{viaje.patente || 'Sin Patente'}</span>
                            <span className="text-[11px] text-slate-400">{viaje.modelo || 'S/M'}</span>
                          </td>
                          <td className="p-4 font-medium text-slate-900">{viaje.conductor_nombre || 'Sin conductor'}</td>
                          <td className="p-4 font-semibold text-slate-600">
                            {new Date(viaje.fecha_hora_salida).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}
                          </td>
                          <td className="p-4">
                            ...
                            <td className="p-4 pr-6 text-right whitespace-nowrap">
                              <div className="flex justify-end gap-2">
                                {viaje.estado === 'programado' && (
                                  <button 
                                    onClick={() => handleStartViaje(viaje.id)}
                                    className="p-1 px-2 bg-blue-600 text-white rounded-md text-xs font-bold hover:bg-blue-700 inline-flex items-center gap-1 shadow-sm transition-all"
                                  >
                                ...
                                {viaje.estado === 'en_progreso' && (
                                  <>
                                    <button 
                                      onClick={() => handleFinishViaje(viaje.id)}
                                      className="p-1 px-2 bg-emerald-600 text-white rounded-md text-xs font-bold hover:bg-emerald-700 inline-flex items-center gap-1 shadow-sm transition-all"
                                    >
                                    ...
                                    <button 
                                      ...
                                      className="p-1 px-2 bg-red-100 text-red-700 rounded-md text-xs font-bold hover:bg-red-200 inline-flex items-center gap-1 transition-all"
                                    >
            ...
            {/* Sidebar de Alertas e Incidentes en Ruta */}
              ...
              <div className="space-y-4 flex-1 overflow-y-auto max-h-[400px] pr-2">
                {alertasActivas.length === 0 ? (
                  <div className="flex flex-col items-center justify-center py-12 text-center">
                    ...
                    <span className="text-sm font-bold text-slate-900">¡Todo en Orden!</span>
                    <span className="text-xs text-slate-400 mt-1">No hay alertas de desvíos, demoras o bloqueos activos.</span>
                  </div>
                ) : (
                  alertasActivas.map((alerta) => (
                    <div key={alerta.id} className="p-4 bg-red-50 border border-red-100 rounded-xl space-y-2 relative shadow-sm">
                      <div className="flex items-center justify-between">
                        <span className="text-xs font-black uppercase text-red-600 bg-red-100 px-2 py-0.5 rounded-md tracking-wider">
                          {alerta.tipo}
                        </span>
                      ...
                      <div className="text-[11px] text-slate-500 font-medium flex items-center gap-1">
                        <Bus size={12} /> {alerta.ruta_nombre} ({alerta.patente})
                      </div>
                      {isAdmin && (
                        <button
                          onClick={() => handleResolveAlerta(alerta.id)}
                          className="mt-2 text-xs font-bold text-emerald-600 hover:text-emerald-700 bg-emerald-50 border border-emerald-200 rounded-md p-1 px-2.5 block text-center w-full shadow-sm hover:shadow transition-all"
                        >
      ...
      {isAlertaModalOpen && selectedViaje && (
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl max-w-md w-full shadow-2xl p-6 relative border border-slate-100 animate-in fade-in zoom-in-95 duration-200">
            <button 
              onClick={() => setIsAlertaModalOpen(false)}
            ...
            <p className="text-xs font-semibold text-slate-500 mb-4">
              Estás reportando un incidente para la ruta <strong className="text-slate-700">{selectedViaje.ruta_nombre}</strong>.
            </p>
            {errorMsg && (
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <textarea
                  ...
                  placeholder="Detalles sobre el incidente (ej: ponchadura de llanta en Av. Juárez, demora estimada 15 mins)."
                  className="w-full p-3 bg-slate-50 border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500 text-sm font-semibold text-slate-700 h-24 resize-none"
                />
              </div>
            </div>
            <div className="flex gap-3 mt-6">
              ...
              <button
                ...
                className="flex-1 py-2.5 bg-red-600 hover:bg-red-700 text-white font-bold rounded-xl text-sm shadow-lg shadow-red-900/10 transition-all"
              >
                Reportar Alerta
              </button>
            </div>
